...
function GM:HUDPaint()
   ...
   if LocalPlayer():Alive() and IsValid(LocalPlayer()) then
	   ...
	   local shake={0,0}
	   local hptc=color_white
	   if(drawhp<20)then
		   shake={math.random(-1,1)*3,math.random(-1,1)*3}
		   hptc=Color(255,0,0)
	   end
	   draw.RoundedBoxEx(6, Spacer * 2, y - (Spacer * 4) - (ySize * 3), Spacer + pHealthWidth, ySize, Color(128, 28, 28, 255), true, true, false, false)
	   draw.SimpleText(math.Max(pHealth, 0).." HP","Flood_HUD_B", xSize * 0.5 + (Spacer * 2) + shake[1], y - (ySize * 2.5) - (Spacer * 4) + shake[2],hptc, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
